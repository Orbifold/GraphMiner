<template>
  <v-container>
    ...
    <v-row v-for="block in data">
      <v-col cols="3" md="3" v-for="item in block">
        <v-card class="mx-auto" max-width="400" color="primary3 " flat>
          ...
          <v-card-actions>
            <v-btn color="primary6" text> Open</v-btn>
...
export default class ProjectsView extends Vue {
  ...
  async addNewProject() {
    const p = await this.$dataService.createProject(`Project ${Utils.randomId()}`);
    await this.refresh();
  }
